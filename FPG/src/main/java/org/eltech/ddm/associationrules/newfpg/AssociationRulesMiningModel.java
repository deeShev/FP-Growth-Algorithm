...
public class AssociationRulesMiningModel extends EMiningModel {
    ...
    /*public int getItemToSet(String itemId) throws MiningException {
        MiningModelElement miningModelElement = getItem();
        if (miningModelElement.size() != 0) {
            for (int i = 0; i < miningModelElement.size(); i++) {
                if (miningModelElement.getElement(i).getID().equals(itemId)){
                    return i;
                }
            }
        }
