...
public class FormTransaction1 extends DataMiningBlock {
    private final String transactionIDsAttributeName;
    private final String itemIDsAttributeName;
    private final String status;
    private final String rootZoneName;
    private final String zoneName;
    private final String numberTS;
     ...
     */
    public FormTransaction1(EMiningFunctionSettings settings) throws MiningException {
        super(settings);
        ...
        rootZoneName = ((AssociationRulesFunctionSettings) settings).getRootZoneName();
        zoneName = ((AssociationRulesFunctionSettings) settings).getZoneName();
        numberTS = ((AssociationRulesFunctionSettings) settings).getNumberTS();
        time = ((AssociationRulesFunctionSettings) settings).getTime();
    }
    ...
    protected EMiningModel execute(MiningInputStream dataSet, EMiningModel model) throws MiningException {
        ...
        String itemId = String.valueOf(miningVector.getValue(transactionIDsAttributeName));

        String typeName = String.valueOf(miningVector.getValue(itemIDsAttributeName));
        String statusID = String.valueOf(miningVector.getValue(status));
        String rootZoneNameID = String.valueOf(miningVector.getValue(rootZoneName));
        String zoneNameID = String.valueOf(miningVector.getValue(zoneName));
        String numberTSID = String.valueOf(miningVector.getValue(numberTS));
        String timeID = String.valueOf(miningVector.getValue(time));
    ...
    private Item adapt(String itemId, Transaction transaction) {
        return new Item(transaction.size() + "_" + itemId.split("_")[1]);
    }
